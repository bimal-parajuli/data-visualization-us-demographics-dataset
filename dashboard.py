...
app.layout = html.Div([
    ...
    html.Div([
        ...
        dcc.Graph(id='strip-dot-chart', style={'width':'250px', 'height':'1000px','margin':'0px'}),
        html.Div([
            html.Div([
                dcc.Graph(id='bar-chart', style={'width':'500px','margin':'0px'}),
                dcc.Graph(id='hierarchical-plot-treemap', style={'width':'500px','margin':'0px'})
            ],style={
            'width':'100%',
            'display':'flex',
            'flex-direction':'row',
            ...
            html.Div([
                dcc.Graph(id='all-state-profession-treemap', style={'width':'500px','margin':'0px'}),
                dcc.Graph(id='choropleth-map', style={'width':'500px','margin':'0px'})
            ],style={
            'width':'100%',
            'display':'flex',
            'flex-direction':'row',
...
@app.callback(
    [dash.dependencies.Output('choropleth-map', 'figure'),
     ...
     dash.dependencies.Output('all-state-profession-treemap', 'figure')],
...
def update_plots(selected_states):
    ...
    all_state_profession = px.treemap(professional_melted_df, path=['StateCode', 'Jobs'], values="TotalPeople")


    return choropleth_map, bar_chart, treemap_chart, strip_chart, all_state_profession
